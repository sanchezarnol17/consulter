...
function renderPatients() {
  ...
  const rows = PSTATE.patients.filter((p) =>
    `${p.nombre || ''} ${p.documento || ''} ${p.telefono || ''}`.toLowerCase().includes(q)
  );
    ...
    div.innerHTML = `
      <div>
        ...
        <div class="text-sm text-gray-500">
          Doc: ${p.documento || '—'} · Tel: ${p.telefono || '—'} · Reg: ${humanDate(p.fecha_creacion)}
        </div>
      </div>
...
async function loadPatients() {
  try {
    ...
    PSTATE.patients = Array.isArray(data) ? data : [];
    renderPatients();
    showNet('');
  } catch (e) {
    $('#list').innerHTML =
      `<div class='text-red-700 bg-red-50 border border-red-200 rounded-xl p-3'>` +
      `No se pudo cargar desde Google Sheets.<br><small>${e.message}</small></div>`;
    showNet('Error');
  }
}
...
function openPatientForm(id) {
  ...
  if (id) {
    const p = PSTATE.patients.find((x) => x.id === id);
    if (p) {
...
const LS_INV = 'inv_products_v3';
const LS_CATS = 'inv_categories_v1';
const LS_DOCS = 'inv_documents_v2';
const LS_HIST = 'inv_history_v1';
...
let CATS = JSON.parse(localStorage.getItem(LS_CATS) || '[]');
let DOCS = JSON.parse(localStorage.getItem(LS_DOCS) || '[]');
let HIST = JSON.parse(localStorage.getItem(LS_HIST) || '[]');

const saveINV = () => localStorage.setItem(LS_INV, JSON.stringify(INV));
const saveCATS = () => localStorage.setItem(LS_CATS, JSON.stringify(CATS));
const saveDOCS = () => localStorage.setItem(LS_DOCS, JSON.stringify(DOCS));
const saveHIST = () => localStorage.setItem(LS_HIST, JSON.stringify(HIST));

// UI comunes
const invSearch = $('#invSearch'),
  invCatFilter = $('#invCatFilter'),
  invList = $('#invList');
...
function refreshCatFilter() {
  ...
  invCatFilter.innerHTML =
    ...
    CATS.map((c) => `<option ${c === chosen ? 'selected' : ''}>${c}</option>`).join('');
}
...
function renderInv() {
  ...
  const rows = INV.filter((p) => {
    ...
    return (!q || txt.includes(q)) && (!cat || p.cat === cat);
  ...
  for (const p of rows) {
    ...
    div.innerHTML = `
      ...
      <div class="flex gap-2">
        <button class="px-3 py-2 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700" data-hist="${p.id}">Historial</button>
        <button class="px-3 py-2 rounded-xl bg-amber-500 text-white hover:bg-amber-600" data-edit="${p.id}">Editar</button>
        <button class="px-3 py-2 rounded-xl bg-red-600 text-white hover:bg-red-700" data-del="${p.id}">Eliminar</button>
      </div>`;
  ...
  invList.querySelectorAll('[data-hist]').forEach((b) => (b.onclick = () => openHist(b.dataset.hist)));
}

// Producto CRUD
function openProdForm(id) {
  const dlg = $('#invProdDialog'),
    f = $('#invProdForm');
  f.reset();
  fillCatSelect();
  $('#prod_id').value = id || '';
...
$('#invProdForm').onsubmit = (e) => {
  ...
  if (i >= 0) {
    ...
    INV.push({ id, name, cat, stock: 0, avg_cost: 0, created_at: new Date().toISOString() });
  }
  saveINV();
  $('#invProdDialog').close();
  renderInv();
...
function deleteProd(id) {
  const p = INV.find((x) => x.id === id);
  if (!p) return;
  if (!confirm(`¿Eliminar "${p.name}"? Esto no borra su historial ni documentos.`)) return;
...
function renderCats() {
  const list = $('#catList');
  list.innerHTML = CATS.length ? '' : '<div class="text-gray-500">Sin categorías.</div>';

  for (const c of CATS) {
    const row = document.createElement('div');
    row.className = 'border rounded-2xl p-3 flex items-center justify-between';
    row.innerHTML = `
      <div>${c}</div>
      <div class="flex gap-2">
        <button class="px-3 py-1 rounded-xl bg-amber-500 text-white hover:bg-amber-600" data-rename="${c}">Renombrar</button>
        <button class="px-3 py-1 rounded-xl bg-red-600 text-white hover:bg-red-700" data-del="${c}">Eliminar</button>
      </div>`;
  ...
  list.querySelectorAll('[data-rename]').forEach(
    (b) =>
      (b.onclick = () => {
        const old = b.dataset.rename;
        const neo = prompt('Nuevo nombre para la categoría', old);
        if (!neo) return;
        if (CATS.includes(neo)) {
          alert('Ya existe esa categoría');
          return;
        }
        CATS = CATS.map((x) => (x === old ? neo : x));
        INV = INV.map((p) => (p.cat === old ? { ...p, cat: neo } : p));
        saveCATS();
        saveINV();
        renderCats();
        renderInv();
...
$('#btnCatClose').onclick = () => $('#catDialog').close();
$('#catForm').onsubmit = (e) => {
  e.preventDefault();
...
function nextDocNo(type) {
  const base = type === 'compra' ? 'C' : 'V';
  const count = DOCS.filter((d) => d.type === type).length + 1;
  return base + String(count).padStart(5, '0');
}

let MOVE_MODE = 'in';
let LINES = []; // {id,product_id,qty,unit_cost}

function openMoveForm(mode) {
  MOVE_MODE = mode;
  LINES = [];
  const dlg = $('#invMoveDialog');
  const f = $('#invMoveForm');
  f.reset();
  $('#invMoveTitle').textContent = mode === 'in' ? 'Compra' : 'Venta';
  $('#doc_type').value = mode === 'in' ? 'compra' : 'venta';
  $('#doc_no').value = nextDocNo(mode === 'in' ? 'compra' : 'venta');
  $('#doc_date').value = today();
  $('#thCost').style.display = mode === 'in' ? 'table-cell' : 'none';
  renderLines();
  dlg.showModal();
}
...
function addLine() {
  if (!INV.length) {
    alert('Primero crea productos.');
    return;
  }
  const id = 'ln_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 5);
  const first = INV[0]?.id || '';
  LINES.push({ id, product_id: first, qty: 1, unit_cost: 0 });
  renderLines();
}
...
function renderLines() {
  ...
  for (const ln of LINES) {
    const prodOpts = INV.map(
      (p) => `<option value="${p.id}" ${ln.product_id === p.id ? 'selected' : ''}>${p.name}</option>`
    ).join('');

    const costCell =
      MOVE_MODE === 'in'
        ? `<input type="number" min="0" step="0.01" class="px-2 py-1 rounded border w-full" value="${ln.unit_cost}" data-field="unit_cost" data-id="${ln.id}">`
        : `<span class="text-gray-400">—</span>`;

    const p = INV.find((x) => x.id === ln.product_id);
    const unit = MOVE_MODE === 'in' ? Number(ln.unit_cost || 0) : Number(p?.avg_cost || 0);
    const subtotal = Number(ln.qty || 0) * Number(unit || 0);

    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td>
        <select class="px-2 py-1 rounded border w-full" data-field="product_id" data-id="${ln.id}">
          ${prodOpts}
        </select>
      </td>
      <td><input type="number" min="1" step="1" class="px-2 py-1 rounded border w-full" value="${ln.qty}" data-field="qty" data-id="${ln.id}"></td>
      <td>${costCell}</td>
      <td>${money(subtotal)}</td>
      <td><button type="button" class="px-2 py-1 rounded bg-red-600 text-white" data-del="${ln.id}">Quitar</button></td>`;
    body.appendChild(tr);
  }
  ...
  body.querySelectorAll('select, input').forEach((el) => {
    el.onchange = (e) => {
      ...
      const val = field === 'qty' ? Number(e.target.value || 0) : field === 'unit_cost' ? Number(e.target.value || 0) : e.target.value;
      setLineValue(id, field, val);
      renderLines();
    };
  });

  body.querySelectorAll('[data-del]').forEach((b) => (b.onclick = () => removeLine(b.dataset.del)));
...
$('#invMoveForm').onsubmit = (e) => {
  ...
  const type = MOVE_MODE === 'in' ? 'compra' : 'venta';
  ...
  if (
    LINES.some(
      (l) => !l.product_id || !l.qty || l.qty <= 0 || (type === 'compra' && (!l.unit_cost || l.unit_cost <= 0))
    )
  ) {
    alert('Verifica cantidades y costos.');
    return;
  }
  ...
  const header = {
    ...
    number: $('#doc_no').value,
    date: $('#doc_date').value,
    party: $('#doc_party').value.trim(),
    notes: $('#doc_notes').value.trim(),
    total: 0,
  ...
  for (const ln of header.lines) {
    ...
    const stock_before = Number(p.stock || 0),
      avg_before = Number(p.avg_cost || 0);

    if (type === 'compra') {
      const unit = Number(ln.unit_cost || 0);
      const newQty = stock_before + Number(ln.qty || 0);
      const newAvg = newQty > 0 ? (stock_before * avg_before + Number(ln.qty || 0) * unit) / newQty : unit;
      p.stock = newQty;
      p.avg_cost = newAvg;
      docTotal += Number(ln.qty || 0) * unit;
      ...
      // venta
      const unit = avg_before;
      if (Number(ln.qty || 0) > stock_before) {
        alert(`Stock insuficiente para ${p.name}`);
        return;
      }
      const newQty = stock_before - Number(ln.qty || 0);
      p.stock = newQty; // el promedio se mantiene
      docTotal += Number(ln.qty || 0) * unit;
...
function openHist(prod_id) {
  const p = INV.find((x) => x.id === prod_id);
  const dlg = $('#historyDialog');
  $('#histProdName').textContent = p ? p.name : '';

  const rows = HIST.filter((h) => h.product_id === prod_id).sort((a, b) =>
    String(b.date || '').localeCompare(String(a.date || ''))
  );

  const list = $('#histList');
  const meta = $('#histMeta');
  meta.textContent = `Stock: ${p?.stock || 0} · Costo prom.: ${money(p?.avg_cost || 0)}`;

  list.innerHTML = rows.length ? '' : '<div class="text-gray-500">Sin movimientos.</div>';

  for (const h of rows) {
    const el = document.createElement('div');
    el.className = 'border rounded-2xl p-3 flex items-center justify-between';
    el.innerHTML = `
      <div>
        <div class="font-semibold">${h.type === 'compra' ? '🟢 Compra' : '🔴 Venta'} — ${h.doc_no} · ${h.date}</div>
        <div class="text-sm text-gray-500">
          Cant: ${h.qty} · Costo unit.: ${money(h.unit_cost || 0)} ·
          Promedio: ${money(h.avg_before || 0)} → <b>${money(h.avg_after || 0)}</b> ·
          Stock: ${h.stock_before} → <b>${h.stock_after}</b>
        </div>
        ${h.party ? `<div class='text-sm'>${h.party}</div>` : ''}
        ${h.notes ? `<div class='text-sm'>${h.notes}</div>` : ''}
      </div>`;
    list.appendChild(el);
  }
...
function openDocs() {
  const dlg = $('#docsDialog');
  const list = $('#docsList');

  const rows = DOCS.slice().sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
  list.innerHTML = rows.length ? '' : '<div class="text-gray-500">Sin documentos.</div>';

  for (const d of rows) {
    const items = d.lines
      .map((ln) => {
        const p = INV.find((x) => x.id === ln.product_id);
        return `${p ? p.name : '(producto)'} × ${ln.qty}${d.type === 'compra' ? ' @ ' + money(ln.unit_cost) : ''}`;
      })
      .join(' · ');
    ...
    el.innerHTML = `
      <div class="flex items-center justify-between">
        <div>
          <div class="font-semibold">${d.type === 'compra' ? '🟢 Compra' : '🔴 Venta'} — ${d.number} · ${d.date}</div>
          <div class="text-sm text-gray-500">${items}</div>
          ${d.party ? `<div class='text-sm'>${d.party}</div>` : ''}
          ${d.notes ? `<div class='text-sm'>${d.notes}</div>` : ''}
        </div>
        <div class="font-bold">${money(d.total)}</div>
      </div>`;
    list.appendChild(el);
  }
...
$('#invCatFilter').onchange = renderInv;
renderInv();

/* ====== INGRESOS/GASTOS (sin cambios) ====== */
const LS_CF = 'cashflow_v1';
let CF = JSON.parse(localStorage.getItem(LS_CF) || '[]');
const saveCF = () => localStorage.setItem(LS_CF, JSON.stringify(CF));
const cfList = $('#cfList');
...
function renderCF() {
  ...
  for (const r of rows) {
    ...
    card.innerHTML = `
      <div>
        <div class="font-semibold">${r.type === 'ingreso' ? '🟢 Ingreso' : '🔴 Gasto'} — ${r.concept || '(sin concepto)'}
          <span class="text-gray-500">· ${r.category || '—'}</span>
        </div>
        <div class="text-sm text-gray-500">${r.date || '—'} · ${r.method || '—'}</div>
...
function openCfForm(id) {
  ...
  if (id) {
    const r = CF.find((x) => x.id === id);
    if (r) {
      $('#cf_date').value = r.date || '';
      $('#cf_type').value = r.type || 'ingreso';
      $('#cf_category').value = r.category || '';
      $('#cf_amount').value = r.amount || '';
      $('#cf_concept').value = r.concept || '';
      $('#cf_method').value = r.method || '';
...
function deleteCF(id) {
  const r = CF.find((x) => x.id === id);
  if (!r) return;
  if (!confirm(`¿Eliminar "${r.type} · ${r.concept}"?`)) return;
  CF = CF.filter((x) => x.id !== id);
  saveCF();
  renderCF();
}
...
$('#cfForm').onsubmit = (e) => {
  ...
  const r = {
    ...
    date: $('#cf_date').value,
    type: $('#cf_type').value,
    category: $('#cf_category').value.trim(),
    amount: Number($('#cf_amount').value || 0),
    concept: $('#cf_concept').value.trim(),
    method: $('#cf_method').value.trim(),
    notes: $('#cf_notes').value.trim()
  };
  if (!r.date || !r.type || !r.category || !r.concept || !r.amount) {
    alert('Completa fecha, tipo, categoría, concepto y monto.');
    return;
  }
  const i = CF.findIndex((x) => x.id === id);
  if (i >= 0) CF[i] = r;
  else CF.push(r);
  saveCF();
  $('#cfDialog').close();
...
$('#cfSearch').oninput = renderCF;
$('#cfType').onchange = renderCF;
$('#cfFrom').onchange = renderCF;
$('#cfTo').onchange = renderCF;
$('#btnCfClear').onclick = () => {
  $('#cfSearch').value = '';
  $('#cfType').value = '';
  $('#cfFrom').value = '';
